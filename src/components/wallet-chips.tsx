...
export function WalletChips({ wallets }: WalletChipsProps) {
  ...
  return (
    <div className="flex gap-1.5 justify-center flex-wrap px-5 pb-4">
      {wallets.map((w) => (
        <span key={w.address} className="text-[9px] text-[var(--trench-text-muted)] backdrop-blur-md px-2 py-1 cut-xs flex items-center gap-1 font-mono" style={{ background: 'rgba(0,212,255,0.04)', border: '1px solid rgba(0,212,255,0.08)' }}>
