...
const Lightspeed: React.FC<LightspeedProps> = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      uniforms.iMouse.value.x = event.clientX - r.left;
      uniforms.iMouse.value.y = actualHeight - (event.clientY - r.top);
    };
    ...
    const handleTouchStart = (event: TouchEvent) => {
      if (event.touches.length > 0) {
        ...
        const r = container.getBoundingClientRect();
        uniforms.iMouse.value.x = touch.clientX - r.left;
        uniforms.iMouse.value.y = actualHeight - (touch.clientY - r.top);
        compressionTargetRef.current = 1;
        uniforms.iMouse.value.z = 1;
      }
    };
    ...
    const animate = (currentTime: number) => {
      ...
      compressionCurrentRef.current +=
        (compressionTargetRef.current - compressionCurrentRef.current) *
        compressionSpeed *
        delta;
