...
export function TradeCard({ tokenSymbol, tokenName, tokenImage, totalPnlPercent, totalPnlSol, transactions }: TradeCardProps) {
  ...
  return (
    <div className="flex-shrink-0 w-[240px] snap-start flex items-start gap-3 p-3.5 cut-sm backdrop-blur-md border border-[rgba(0,212,255,0.08)] hover:border-[rgba(0,212,255,0.2)] transition-colors"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-col gap-1">
          {transactions.slice(0, 4).map((tx, i) => (
            <div key={i} className="flex justify-between items-center text-[9px] px-1.5 py-1 cut-xs backdrop-blur-sm" style={{ background: 'rgba(0,212,255,0.02)' }}>
